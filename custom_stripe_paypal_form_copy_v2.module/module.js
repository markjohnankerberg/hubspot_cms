...
function renderProducts(products) {
    ...
    area.innerHTML = `<h3 class="product_area__title product_area__title--centered">All Gifts are Double Matched</h3><h3 class="product_area__title">Product Summary</h3>`;
    ...
    products.forEach((p, i) => {
      ...
      <div class="product_match_popup">
        <div class="match_formula">
          <img class="match_formula_icon" src="${iconUrl}" alt="${p.name}" />
          <span class="match_formula_text"></span>
        </div>
        <p class="match_label">Triple Matched by the Ministry</p>
      </div>
    `;
        ...
        const matchPopup = row.querySelector(".product_match_popup");
        ...
        const updateMatchPopup = () => {
            const qty = parseInt(qtyInput.value || "0", 10);
            matchPopup.classList.toggle("is-visible", qty > 0);
            if (qty > 0) {
                const devicesPerUnit = p.sku === "FR16-1000" ? 2 : 1;
                matchPopup.querySelector(".match_formula_text").textContent = `+ Double Match = ${qty * 3 * devicesPerUnit} Devices Delivered`;
                matchPopup.querySelector(".match_label").textContent = "";
            }
        };
...
window.addEventListener("load", () => {
    ...
    const monthlyNote = document.getElementById("monthly-match-note");
    document.querySelectorAll('input[name="donation_type"]').forEach(radio => {
        radio.addEventListener("change", () => {
            if (monthlyNote) monthlyNote.style.display = isDonationTypeMonthly() ? "block" : "none";
        });
    });
});
